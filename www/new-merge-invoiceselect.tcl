...
set target_cost_type $cost_type
...
db_foreach invoices_info_query $selection {
    ...
    # Find out if there is already a linked document of the target type.
    # Ignore that invoice from the list.
    ...
    ds_comment "$cost_id :: $linked_invoice_ids"
...
db_1row company_get_info "select
        c.*,
...
" {
    ...
    if {"" != [string trim $value]} {
        ...
        append dynfield_attributes_html "
        <tr $bgcolor>
          <td>$attrib_name</td>
          <td>$value</td>
        </tr>
